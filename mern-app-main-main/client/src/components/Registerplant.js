...
// Mapeamento necessário para associar cada campo à sua respectiva coleção no Atlas e label do Modal
const mapeamentoColecoes = {
    fruit: { colecao: "fruits", label: "Fruto" },
    origin: { colecao: "origins", label: "Origem" },
    type: { colecao: "types", label: "Tipo" },
    propagation: { colecao: "propagations", label: "Propagação" },
    toxicity: { colecao: "toxicities", label: "Toxicidade" },
    dificulty: { colecao: "dificulties", label: "Dificuldade" },
    height: { colecao: "heights", label: "Altura" },
    flowercolor: { colecao: "flowercolors", label: "Cor da Flor" },
    foliage: { colecao: "foliages", label: "Folhagem" },
    flowering: { colecao: "flowering", label: "Floração" },
    light: { colecao: "lights", label: "Luminosidade" },
    water: { colecao: "water", label: "Necessidade de Água" },
    size: { colecao: "sizes", label: "Tamanho do Vaso/Solo" },
    soil: { colecao: "soils", label: "Tipo de Solo" },
    manha: { colecao: "manhas", label: "Melhor Horário de Rega" },
    amount: { colecao: "amounts", label: "Quantidade de Água" },
    frequency: { colecao: "frequencies", label: "Frequência de Fertilização" },
    NPK: { colecao: "NPKs", label: "NPK Recomendado" },
    season: { colecao: "seasons", label: "Época de Poda" },
    tools: { colecao: "tools", label: "Ferramentas de Poda" },
    prevention: { colecao: "preventions", label: "Nível de Prevenção" },
    monitoring: { colecao: "monitoring", label: "Monitoramento de Pragas" },
    station: { colecao: "stations", label: "Estação de Plantio" },
    spacing: { colecao: "spacings", label: "Espaçamento entre Mudas" },
    iluminosity: { colecao: "iluminosity", label: "Sol Diário" },
    protection: { colecao: "protections", label: "Proteção de Cultivo" },
    idealTemperature: { colecao: "idealTemperatures", label: "Temperatura Ideal" },
    tolerance: { colecao: "tolerances", label: "Tolerância" }
}
...
export default function RegisterPlant() {
    const [form, setForm] = useState({
        ...
        prevention: "", monitoring: "", planting: "", exhibition: "", maintenance: "",
    ...
    // Envia um novo item customizado para o banco de dados através do Modal
    ...
    // Cadastra a planta inteira no banco
    async function handleRegister(e) {
        e.preventDefault()
        setMensagem("")

        try {
            const response = await fetch(`${REACT_APP_YOUR_HOSTNAME}/plant/register`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
            ...
            setTimeout(() => { navigate("/plantlist") }, 1000)
    ...
    // Componente Interno Helper para renderizar os Selects Dinâmicos com o botão "+"
    function RenderSelectComBotaoPlus({ campo, placeholder, filhosPadrao }) {
        return (
            <div className="input-group">
                ...
                >
                    ...
                    {filhosPadrao}
                    {opcoesBanco[campo] && opcoesBanco[campo].map((opt) => (
                ...
                <div className="tab-content" id="plantFormTabsContent">
                    ...
                    <div className="tab-pane fade show active" id="dados-basicos" role="tabpanel">
                        <div className="row">
                            <div className="col-md-6 mb-3">
                                ...
                                <input type="text" placeholder="Exemplo: Rosa do Deserto." className="form-control" value={form.name} onChange={(e) => updateForm({ name: e.target.value })} />
                            </div>
                            <div className="col-md-6 mb-3">
                                <label>Nome Científico</label>
                                <input type="text" placeholder="Exemplo: Adenium obesum." className="form-control" value={form.scientificName} onChange={(e) => updateForm({ scientificName: e.target.value })} />
                            </div>
                            <div className="col-md-12 mb-3">
                                <label>Descrição Simples</label>
                                <textarea className="form-control" placeholder="Breve descrição das características mais marcantes..." rows="3" maxLength="300" value={form.simpleDescription} onChange={(e) => updateForm({ simpleDescription: e.target.value })} />
                            </div>
                            <div className="col-md-12 mb-3">
                                <label>Descrição Completa</label>
                                <textarea className="form-control" placeholder="Descrição detalhada sobre a aparência, habitat..." rows="4" value={form.description} onChange={(e) => updateForm({ description: e.target.value })} />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="tab-pane fade" id="info-botanicas" role="tabpanel">
                        <div className="row">
                            ...
                            <div className="col-md-4 mb-3"><label>Filo</label><input type="text" className="form-control" placeholder="Ex: Tracheophyta." value={form.Filo} onChange={(e) => updateForm({ Filo: e.target.value })} /></div>
                            <div className="col-md-4 mb-3"><label>Classe</label><input type="text" className="form-control" placeholder="Ex: Magnoliopsida." value={form.Classe} onChange={(e) => updateForm({ Classe: e.target.value })} /></div>
                            <div className="col-md-4 mb-3"><label>Ordem</label><input type="text" className="form-control" placeholder="Ex: Gentianales." value={form.Ordem} onChange={(e) => updateForm({ Ordem: e.target.value })} /></div>
                            <div className="col-md-4 mb-3"><label>Família</label><input type="text" className="form-control" placeholder="Ex: Apocynaceae." value={form.Family} onChange={(e) => updateForm({ Family: e.target.value })} /></div>
                            <div className="col-md-4 mb-3"><label>Gênero</label><input type="text" className="form-control" placeholder="Ex: Adenium." value={form.Genero} onChange={(e) => updateForm({ Genero: e.target.value })} /></div>
                            <div className="col-md-4 mb-3"><label>Espécie</label><input type="text" className="form-control" placeholder="Ex: Adenium obesum." value={form.Especie} onChange={(e) => updateForm({ Especie: e.target.value })} /></div>
                        </div>
                    </div>
                    ...
                    <div className="tab-pane fade" id="cuidados" role="tabpanel">
                        <div className="row">
                            ...
                            <div className="col-md-12 mb-3 mt-3">
                                ...
                                <textarea className="form-control" placeholder="Descrição detalhada sobre pragas e doenças..." rows="3" value={form.pests} onChange={(e) => updateForm({ pests: e.target.value })} />
                            </div>
                            <div className="col-md-6 mb-3">
                                <label>Prevenção</label>
                                <RenderSelectComBotaoPlus campo="prevention" placeholder="Qual o nível de prevenção?" filhosPadrao={[
                                ]} />
                            </div>
                            <div className="col-md-6 mb-3">
                    ...
                    <div className="tab-pane fade" id="cultivo" role="tabpanel">
                        <div className="row">
                            ...
                            <div className="col-md-6 mb-3">
                                <label>Estação</label>
                                <RenderSelectComBotaoPlus campo="station" placeholder="Qual a melhor estação?" filhosPadrao={[
                                ]} />
                            </div>
                            <div className="col-md-6 mb-3">
